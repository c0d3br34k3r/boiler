...
class BuildDocs {
	...
	private static void processDirectory(CarrotEngine engine, int depth, String inputDir,
			String outputDir) {
		...
		for (File inf : children) {
			if (inf.isDirectory()) {
				...
				processDirectory(engine, depth + 1, inf.getAbsolutePath(),
						outputDir + File.separator + inf.getName());
			} else {
				processFile(engine, depth, inf.getAbsolutePath(),
						outputDir + File.separator + inf.getName());
			}
		}
	}

	private static void processFile(CarrotEngine engine, int depth, String inputFile,
			String outputFile) {
