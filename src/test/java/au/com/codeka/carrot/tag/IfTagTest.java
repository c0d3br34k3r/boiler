...
public class IfTagTest {
	...
	public void testLongValues() throws CarrotException {
		assertThat(render("{% if foo < 10000000000000 %}yes{% end %}", "foo", 9999999999999L))
				.isEqualTo("yes");
		assertThat(render("{% if foo < 10000000000000 %}yes{% end %}", "foo", 10000000000000L))
				.isEqualTo("");
		assertThat(render("{% if foo < 10000000000000 %}yes{% end %}", "foo", 10000000000001L))
				.isEqualTo("");
		assertThat(render("{% if foo > 10000000000000 %}yes{% end %}", "foo", 9999999999999L))
				.isEqualTo("");
		assertThat(render("{% if foo > 10000000000000 %}yes{% end %}", "foo", 10000000000000L))
		...
		assertThat(render("{% if foo >= 10000000000000 %}yes{% end %}", "foo", "10000000000000"))
				.isEqualTo("yes");
	}
	...
	public void testDifferentIntegerTypesEquality() throws CarrotException {
		assertThat(render("{% if foo == bar %}yes{% end %}", "foo", 123, "bar", 123L))
				.isEqualTo("yes");
		assertThat(render("{% if foo == bar %}yes{% end %}", "foo", 123L, "bar", 123))
				.isEqualTo("yes");
	...
	public void testIfIn() throws CarrotException {
		assertThat(render("{% if foo in (1,2,3) %}yes{% else %}no{% end %}", "foo", 2L))
				.isEqualTo("yes");
		assertThat(render("{% if foo in (1,2,3) %}yes{% else %}no{% end %}", "foo", 4L))
				.isEqualTo("no");
		assertThat(render("{% if foo in list %}yes{% else %}no{% end %}", "foo", 2, "list",
				Arrays.asList(1, 2, 3))).isEqualTo("yes");
		assertThat(render("{% if foo in list %}yes{% else %}no{% end %}", "foo", 4, "list",
				Arrays.asList(1, 2, 3))).isEqualTo("no");
	}

	@Test
	public void testNulls() throws CarrotException {
		assertThat(render("{% if foo == null %}yes{% end %}", "foo", null)).isEqualTo("yes");
		assertThat(render("{% if foo == bar %}yes{% end %}", "foo", null, "bar", null))
				.isEqualTo("yes");
		assertThat(render("{% if foo == bar %}yes{% end %}", "foo", new Object(), "bar", null))
				.isEqualTo("");
		assertThat(render("{% if foo != bar %}yes{% end %}", "foo", new Object(), "bar", null))
				.isEqualTo("yes");
		assertThat(render("{% if foo != null %}yes{% end %}", "foo", new Object(), "bar", null))
				.isEqualTo("yes");
