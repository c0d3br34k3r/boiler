...
public class ForTagTest {
	...
	public void testIterableLoopExpansion() throws CarrotException {
		Map<String, Object> context = new HashMap<>();
		assertThat(
				render("Hello {% for number, letter in (1, \"a\"), (2, \"b\"), (3, \"c\") %} {{ number }}:{{ letter }} {% end %} World",
						context))
	...
	public void testSingleLoopExpansion() throws CarrotException {
		assertThat(render("Hello {% for number in 1,  %} {{ number }} {% end %} World",
				new HashMap<String, Object>())).isEqualTo("Hello  1  World");
		assertThat(render("Hello {% for number in 1, 2 %} {{ number }} {% end %} World",
				new HashMap<String, Object>())).isEqualTo("Hello  1  2  World");
		assertThat(render("Hello {% for number in 1, 2, %} {{ number }} {% end %} World",
				new HashMap<String, Object>())).isEqualTo("Hello  1  2  World");
	}
	...
	public void testMapExpansionLoop() throws CarrotException {
		...
		// note, we can not test Map iteration with more values, because the
		// iteration order is undefined
		values.put("foo", "a");
		context.put("values", new MapBindings(values));
		assertThat(
				render("Hello {% for key, val in values %} -{{ key }}:{{ val }}- {% end %} World",
						context))
	...
	public void testKeyValueInMap() throws CarrotException {
		...
		context.put("values", values);
		assertThat(
				render("Hello {% for k, v in values %} -{{ k }}/{{ v }}- {% end %} World", context))
	...
	private String render(String content, @Nullable Map<String, Object> bindings)
