 ...
 * @author Marten Gajda
 */
public final class AccessTerm implements Term {
	...
	public Object evaluate(Configuration config, Scope scope) throws CarrotException {
		return Access.access(term.evaluate(config, scope), new LazyTerm(config, scope, accessor));
	}
	...
	// @Nonnull
	// @Override
	// public Callable callable(@Nonnull final Configuration config, @Nonnull
	// final Scope scope)
	...
	// @Nullable
	// @Override
	// public Object call(@Nonnull Iterable<?> args) throws CarrotException {
	//
	...
	//
	// try {
	// Method method = findMethod(config, value.getClass(), right.toString(),
	...
	//
	// /**
	// * Attempts a basic sort of overload matching, looking for a method
	// * on the given class that we can call.
	// * <p>
	// * <p>
	// * Unlike normal Java overload resolution, we just return the
	// * <em>first</em> method that matches close enough. So if there's a
	// * method that takes an int and one that takes a long (say), we make
	// * no guarantee about which one will be returned.
	// * </p>
	// *
	// * @param config The {@link Configuration}.
	// * @param cls The {@link Class} to search for a method for.
	// * @param name The name of the function we want to search for (not
	// * case sensitive, unlike normal Java).
	// * @param paramTypes The types of the parameters we have. We'll
	// * attempt to find a method that matches the given types as
	// * closely as possible (for example, a method that takes an
	// * int is as good as one that takes longs).
	// * @param paramValues The values we're going to execute. If the
	// * types don't match exactly, we may need to convert some of
	// * the parameters first.
	// * @return The {@link Method} that matches the given name and
	// * parameter values.
	...
	//
	// // This is a list of "candidate" methods that we tried and
	// // rejected for whatever reason. Useful in the error
	...
	// Log.debug(config, "Wrong number of arguments: %d: %s", paramTypes.size(),
	// method);
	...
	// // Special case for passing null values in, any nullable
	// // type is acceptable
	// if (paramValues.get(i) == null) {
	// if (!methodParamTypes[i].isPrimitive()) {
	// // TODO: only if it's decorated with @Nonnull or
	// // something should be disallow this.
	// continue;
	// }
	// }
	// if (!methodParamTypes[i].isAssignableFrom(paramTypes.get(i))) {
	// Object convertedValue =
	// convertType(methodParamTypes[i], paramValues.get(i));
	...
	// for (int i = 0; i < methodParamTypes.length; i++) {
	// paramValues.set(i,
	// convertType(method.getParameterTypes()[i], paramValues.get(i)));
	// }
	// return method;
	// }
	// }
	//
	// // ArrayList inherits `toString()` from AbstractCollection which
	// // renders the elements as a comma separated list
	// // in "[ ]".
	// // see
	// //
	...
	//
	// /**
	// * Attempts to perform a conversion from the given value to the
	// * given output type.
	// *
	// * @param outputType The output type that we want.
	// * @param value The value that we want to convert.
	// * @return The converted value, or null if no conversion is
	// * possible.
	// */
	// @Nullable
