...
class BindingCreator {
	...
	Bindings create(Configuration config, Scope scope) throws CarrotException {
		switch (entries.size()) {
			...
			case 1:
				BindingEntry entry = entries.get(0);
				return new SingletonBindings(entry.name, entry.value.evaluate(config, scope));
			default:
				return buildMapBindings(config, scope);
		}
	}

	private Bindings buildMapBindings(Configuration config, Scope scope) throws CarrotException {
