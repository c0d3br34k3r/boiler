 ...
 *
 * <p>
 * The contents of the second file will then be the contents of the skeleton
...
public class ExtendsTag extends Tag {
	...
	public void render(CarrotEngine engine, Writer writer, TagNode tagNode, Scope scope)
		...
		// We take our children, which should all be block tags, add them to a
		// special variable in the scope, and then
		...
		for (Node childNode : tagNode.getChildren()) {
			...
			if (!(childTagNode.getTag() instanceof BlockTag)) {
				Log.warning(engine.getConfig(), "Unexpected tag instde {%% extends %%}: {%% %s %%}",
						childTagNode.getTag());
				continue;
			}
