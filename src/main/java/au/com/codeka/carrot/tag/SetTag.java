...
public class SetTag extends Tag {
	...
	public void render(CarrotEngine engine, Writer writer, TagNode tagNode, Scope scope)
		...
		if (expression == null) {
			...
			scope.extendCurrent(
					new SingletonBindings(identifiers.get(0).evaluate(), stringWriter.toString()));
		} else if (identifiers.size() == 1) {
			scope.extendCurrent(
					new SingletonBindings(identifiers.get(0).evaluate(),
							expression.evaluate(engine.getConfig(), scope)));
		} else {
			scope.extendCurrent(
					new IterableExpansionBindings(identifiers,
							(Iterable<?>) expression.evaluate(engine.getConfig(), scope)));
		}
	}
}
